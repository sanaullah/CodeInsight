"""
Reusable node functions for LangGraph workflow.
...
from workflow.swarm_analysis_langfuse import get_node_phase
...
async def llm_node(state: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if "tools" in kwargs:
             llm = infrastructure.llm.bind_tools(kwargs["tools"], tool_choice=kwargs.get("tool_choice"))
        ...
        response = await infrastructure.llm.ainvoke(langchain_messages, config=config)
