...
from workflow.swarm_analysis.tools import get_tool_definitions, execute_tool
from workflow.swarm_analysis.tools.tool_executor import parse_tool_call
from infrastructure.utils.model_capabilities import check_tool_calling_support
from infrastructure.utils.langfuse.tracing import create_observation

# from workflow.swarm_analysis.mcp.filesystem_client import FilesystemMCPClient # Removed: MCP archived
...
def spawn_agents_node(state: SwarmAnalysisState) -> SwarmAnalysisState:
    ...
    try:
        ...
        for role_obj in selected_roles:
            ...
            try:
                from infrastructure.utils.langfuse.prompts import get_langfuse_prompt_by_name
                ...
                from infrastructure.utils.langfuse.prompts import normalize_label
        ...
        logger.info(f"Spawned {len(agents)} analysis.agents. Authorized roles: {authorized_roles}")
...
async def execute_single_agent_node(
    ...
    try:
        if enable_chunking and files:
            ...
            from analysis.chunking import ChunkManager, ChunkingStrategy
            from indexing.scanners.project_scanner import FileInfo
            ...
            from analysis.chunking.token_counter import TokenCounter
            ...
            if total_tokens <= max_tokens_per_chunk and len(files) > 0:
                ...
                from analysis.chunking.chunk_manager import FileChunk
            ...
            if len(chunks) > 1:
                ...
                try:
                    from infrastructure.utils.langfuse.evaluation import evaluate_prompt_effectiveness
...
def collect_agent_results_node(state: SwarmAnalysisState) -> SwarmAnalysisState:
    ...
    if not agent_results_list:
        ...
        if agents:
            error_msg = (
                ...
                f"Expected results from {len(agents)} analysis.agents. "
    ...
    try:
        ...
        error_msg = (
            ...
            f"Received {len(agent_results_list)} results from {len(agents) if agents else 0} expected analysis.agents. "
