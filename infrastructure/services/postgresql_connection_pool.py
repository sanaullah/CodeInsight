...
class PostgreSQLConnectionPool:
    ...
    def _get_connection_params(
        self, db_name: str, read_only: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def _create_pool(
        ...
        try:
            ...
            if maxconn <= minconn:
                logger.warning(
                    f"POSTGRES_POOL_MAXCONN ({maxconn}) must be > POSTGRES_POOL_MINCONN ({minconn}), using default 20"
                )
                maxconn = 20
            ...
            if maxconn > 100:
                logger.warning(
                    f"POSTGRES_POOL_MAXCONN ({maxconn}) exceeds recommended limit of 100, capping at 100"
                )
                maxconn = 100

        except (ValueError, TypeError) as e:
            logger.warning(
                f"Invalid pool configuration (minconn/maxconn), using defaults: {e}"
            )
            minconn = 1
            maxconn = 20
        ...
        pool_instance = pool.SimpleConnectionPool(
            minconn=minconn, maxconn=maxconn, **params
        )

        if pool_instance:
            logger.info(
                f"Created PostgreSQL connection pool for {connection_key} (minconn={minconn}, maxconn={maxconn})"
            )
        else:
    ...
    def get_connection(self, db_name: str, read_only: bool = False):
        ...
        with self._pool_lock:
            ...
            pool_size = (
                pool_instance.maxconn if hasattr(pool_instance, "maxconn") else None
            )

        # Track connection acquisition
        try:
            from infrastructure.utils.langfuse.db_tracking import track_db_connection
            from infrastructure.utils.langfuse.tracing import get_current_trace_context
            ...
            with track_db_connection(
                ...
                active_connections=len(
                    [p for p in self._pools.values() if hasattr(p, "_used")]
                )
        ...
        try:
            ...
            if conn is None:
                raise RuntimeError(
                    f"Failed to get connection from pool: {connection_key}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Connection test failed for {connection_key}: {e}, returning connection to pool"
                )
            ...
            logger.error(
                f"Error using connection for {connection_key}: {e}", exc_info=True
            )
    ...
    def close_pool(self, db_name: str, read_only: Optional[bool] = None) -> None:
        ...
        with self._pool_lock:
            if read_only is not None:
                ...
                keys_to_remove = [
                    key for key in self._pools.keys() if key.startswith(f"{db_name}:")
                ]
...
# New implementations should use infrastructure.services.health_checks.postgresql_health.PostgreSQLHealthCheck
class PostgreSQLHealthCheck:
    ...
    def check_health(self):
        ...
        """
        try:
            from infrastructure.services.health_check import ServiceHealth, HealthStatus
        except ImportError:
                ...
                with conn.cursor() as cursor:
                    ...
                    if result and result[0] == 1:
                        ...
                        return ServiceHealth(
                            ...
                            details={"db_name": self.db_name, "pool_count": pool_count},
