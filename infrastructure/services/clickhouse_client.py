...
from infrastructure.services.db_config import get_db_config, ClickHouseConfig
...
class ClickHouseClient:
    ...
    def _get_client(self) -> Any:
        ...
        if self._client is None or not self._connected:
            try:
                ...
                logger.info(
                    f"ClickHouse client connected to {self.config.host}:{self.config.native_port}"
                )
    ...
    def create_table(
        self, table: str, schema: str, database: Optional[str] = None
    ) -> None:
        """
