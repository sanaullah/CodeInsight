...
class RedisClient:
    ...
    Uses configuration from infrastructure.services.db_config.
    """
    ...
    def _record_failure() -> None:
        ...
        with _circuit_lock:
            ...
            if _circuit_failures >= CIRCUIT_FAILURE_THRESHOLD:
                ...
                logger.warning(
                    f"Circuit breaker opened after {_circuit_failures} failures"
                )
        ...
        with _circuit_lock:
            if _circuit_state == CircuitState.CLOSED:
                ...
                if (
                    ...
                    and (time.time() - _circuit_last_failure_time) >= CIRCUIT_TIMEOUT
                ):
    ...
    def get_client() -> Optional[Any]:
        ...
        if (
            ...
            and not RedisClient.is_circuit_open()
        ):
            try:
                # Quick health check
                _redis_client.ping()
        ...
        if _redis_config is None:
            try:
                from infrastructure.services.db_config import get_db_config
        ...
        for attempt in range(max_retries):
            try:
                ...
                RedisClient._record_success()
                logger.info(
                    f"Redis client connected to {_redis_config.host}:{_redis_config.port}"
                )
                return _redis_client
    ...
    def get_stream_maxlen() -> int:
        ...
        return (
            getattr(_redis_config, "stream_maxlen", 10000) if _redis_config else 10000
        )
    ...
    def cache_delete_by_prefix(prefix: str) -> int:
        ...
        if client is None:
            logger.debug(
                f"Redis not available, skipping cache delete by prefix: {prefix}"
            )
            return 0

        global_prefix = RedisClient.get_key_prefix()
        full_prefix = (
            prefix if prefix.startswith(global_prefix) else f"{global_prefix}{prefix}"
        )
    ...
    def cache_delete_by_pattern(pattern: str) -> int:
        ...
        if client is None:
            logger.debug(
                f"Redis not available, skipping cache delete by pattern: {pattern}"
            )
            return 0
    ...
    def cache_batch_delete(keys: List[str]) -> int:
        ...
        if client is None:
            logger.debug(
                f"Redis not available, skipping cache batch delete for {len(keys)} keys"
            )
            return 0
        ...
        prefix = RedisClient.get_key_prefix()
        full_keys = [
            key if key.startswith(prefix) else f"{prefix}{key}" for key in keys
        ]
        ...
        try:
            ...
            while len(keys) < count:
                cursor, found_keys = client.scan(
                    cursor, match=full_pattern, count=count
                )
