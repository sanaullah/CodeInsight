...
from infrastructure.services.health_check import HealthCheck, ServiceHealth, HealthStatus
...
class HealthMonitor:
    """
    Centralized health monitor for all infrastructure.services.
    ...
    def _register_default_checks(self) -> None:
        ...
        try:
            from infrastructure.services.health_checks.postgresql_health import PostgreSQLHealthCheck
        ...
        try:
            from infrastructure.services.health_checks.redis_health import RedisHealthCheck
        ...
        # try:
        #     from infrastructure.services.health_checks.clickhouse_health import ClickHouseHealthCheck
        #     self.register_health_check(ClickHouseHealthCheck())
        # except Exception as e:
        #     logger.warning(f"Could not register ClickHouse health check: {e}")

        try:
            from infrastructure.services.health_checks.minio_health import MinIOHealthCheck
        ...
        try:
            from infrastructure.services.health_checks.langfuse_health import LangfuseHealthCheck
        ...
        try:
            from infrastructure.services.health_checks.llm_health import LLMHealthCheck
    ...
    def get_service_health(self, service_name: str) -> Optional[ServiceHealth]:
        ...
        try:
            ...
            logger.error(
                f"Error checking health for {service_name}: {e}", exc_info=True
            )
            return ServiceHealth(
                status=HealthStatus.UNKNOWN, message=f"Error checking health: {str(e)}"
            )

    def check_all_services(self) -> Dict[str, ServiceHealth]:
        """
        Check health of all registered infrastructure.services.
    ...
    def get_health_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "unknown_count": sum(
                1 for h in all_health.values() if h.status == HealthStatus.UNKNOWN
            ),
        }
