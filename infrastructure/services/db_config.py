...
class PostgreSQLConfig:
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if not (1 <= self.port <= 65535):
            return (
                False,
                f"PostgreSQL port must be between 1 and 65535, got {self.port}",
            )
        if not self.user:
...
class ClickHouseConfig:
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if not (1 <= self.http_port <= 65535):
            return (
                False,
                f"ClickHouse HTTP port must be between 1 and 65535, got {self.http_port}",
            )
        if not (1 <= self.native_port <= 65535):
            return (
                False,
                f"ClickHouse native port must be between 1 and 65535, got {self.native_port}",
            )
        if not self.user:
...
class MinIOConfig:
    ...
    def from_env(cls) -> "MinIOConfig":
        ...
        secret_key = (
            os.getenv("MINIO_SECRET_KEY") or os.getenv("MINIO_ROOT_PASSWORD") or ""
        )
        if not secret_key:
            raise ValueError(
                "MINIO_SECRET_KEY or MINIO_ROOT_PASSWORD environment variable is required"
            )
        ...
        return cls(
            ...
            force_path_style=os.getenv("MINIO_FORCE_PATH_STYLE", "true").lower()
            == "true",
        )
...
class DatabaseConfig:
    """Aggregated database configuration for all infrastructure.services."""
