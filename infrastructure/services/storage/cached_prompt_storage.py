...
from infrastructure.services.storage.prompt_storage import PromptStorage
...
class CachedPromptStorage(CachedStorage):
    ...
    def get_prompt_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                f"Invalid cache key for prompt name: {e}, using sanitized key"
            )
    ...
    def get_all_prompts(
        ...
        try:
            ...
            if cached is not None:
                logger.debug(
                    f"Cache hit for get_all_prompts (limit={limit}, offset={offset})"
                )
                return cached
        except Exception as e:
