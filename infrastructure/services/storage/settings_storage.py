...
from infrastructure.services.storage.base_storage import BaseStorage, StorageError
...
class SettingsStorage(BaseStorage):
    ...
    def get(self, record_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if len(parts) != 2:
                raise StorageError(
                    f"Invalid record_id format: {record_id}. Expected 'user_id:setting_key'"
                )
            ...
            with get_db_connection(self.db_identifier, read_only=True) as conn:
                with conn.cursor() as cursor:
                    ...
                    if setting.get("setting_value"):
                        try:
                            setting["setting_value"] = _json_loads(
    ...
    def get_setting(
        ...
        try:
            ...
            logger.error(
                f"Error getting setting {key} for user {user_id}: {e}", exc_info=True
            )
            return default
    ...
    def set_setting(self, user_id: str, key: str, value: Any) -> bool:
        ...
        """
        try:
            data = {"user_id": user_id, "setting_key": key, "setting_value": value}
            self.save(data)
    ...
    def get_all_settings(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error getting all settings for user {user_id}: {e}", exc_info=True
            )
            return {}
    ...
    def delete_user_settings(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error deleting settings for user {user_id}: {e}", exc_info=True
            )
            return False
    ...
    def delete_setting(self, user_id: str, key: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error deleting setting {key} for user {user_id}: {e}", exc_info=True
            )
            return False
