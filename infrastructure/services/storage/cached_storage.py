...
from infrastructure.services.cache_utils import (
...
class CachedStorage:
    """
    Generic cache wrapper for storage infrastructure.services.
    ...
    def _get_with_fallback(
        self, record_id: str, obs: Optional[Any] = None
    ...
    def save(self, data: Dict[str, Any]) -> str:
        ...
        try:
            from infrastructure.utils.langfuse.db_tracking import track_storage_operation
            ...
            logger.debug(
                f"Failed to track cached storage save operation: {track_error}"
            )
        ...
        try:
            ...
            with track_storage_operation(
                ...
                if obs:
                    try:
                        obs.update(
                            metadata={"cache_hit": False, "result_count": len(results)}
                        )
                    except Exception:
                        pass
            ...
            logger.debug(
                f"Failed to track cached storage query operation: {track_error}"
            )
            # Fallback to original logic
            if not self.enable_query_cache:
