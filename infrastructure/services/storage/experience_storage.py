...
from infrastructure.services.storage.base_storage import (
    BaseStorage,
    StorageError,
    StorageNotFoundError,
)
from infrastructure.services.db_config import get_db_config, CODELUMEN_DATABASE
from infrastructure.services.postgresql_connection_pool import get_db_connection, transaction
...
try:
    ...
    _json_dumps = lambda obj: orjson.dumps(obj, option=orjson.OPT_NON_STR_KEYS).decode(
        "utf-8"
    )
    _USE_ORJSON = True
except ImportError:
...
class ExperienceStorage(BaseStorage):
    ...
    def save(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            goal_understanding_json = _json_dumps(
                self._serialize_goal_understanding(experience.goal_understanding)
            )
            strategy_used_json = _json_dumps(
                self._serialize_strategy(experience.strategy_used)
            )
            outcome_json = _json_dumps(self._serialize_outcome(experience.outcome))
            performance_metrics_json = _json_dumps(
                self._serialize_performance_metrics(experience.performance_metrics)
            )

            # Extract success_level from outcome
            success_level = (
                experience.outcome.success_level.value
                if experience.outcome.success_level
                else None
            )
    ...
    def query(
        ...
        try:
            ...
            where_clause = (
                sql.SQL(" AND ").join(map(sql.SQL, where_clauses))
                if where_clauses
                else sql.SQL("1=1")
            )
            ...
            """).format(
                ...
                order=sql.SQL(
                    order_by
                ),  # Danger: blindly trusting order_by string, but it's consistent with previous f-string. Ideally validation needed.
            )
            ...
            if query_time > 1.0:
                logger.warning(
                    f"Slow query detected: {query_time:.2f}s for query with filters: {filters}"
                )
    ...
    def get_similar_experiences(
        self, experience: Experience, limit: int = 10
    ) -> List[Experience]:
        """
    ...
    def get_experience_statistics(
        ...
        try:
            ...
            return {"total_experiences": count, "agent_name": agent_name or "all"}
    ...
    def _serialize_goal_understanding(self, goal) -> Dict[str, Any]:
        ...
        try:
            from analysis.agents.autonomous_models import GoalUnderstanding

            if isinstance(goal, GoalUnderstanding):
                return {
                    "primary_objective": goal.primary_objective,
                    "sub_goals": [
                        {"goal_id": sg.goal_id, "description": sg.description}
                        for sg in goal.sub_goals
                    ],
                    "priority": goal.priority.value
                    if hasattr(goal.priority, "value")
                    else str(goal.priority),
                    "estimated_complexity": goal.estimated_complexity.value
                    if hasattr(goal.estimated_complexity, "value")
                    else str(goal.estimated_complexity),
    ...
    def _serialize_strategy(self, strategy) -> Dict[str, Any]:
        ...
        try:
            from analysis.agents.autonomous_models import Strategy
    ...
    def _serialize_outcome(self, outcome) -> Dict[str, Any]:
        ...
        if isinstance(outcome, Outcome):
            return {
                ...
                if hasattr(outcome.success_level, "value")
                else str(outcome.success_level),
    ...
    def _serialize_adaptation(self, adaptation) -> Dict[str, Any]:
        ...
        try:
            from analysis.agents.autonomous_models import AdaptationRecord

            if isinstance(adaptation, AdaptationRecord):
                return {
                    "adaptation_id": adaptation.adaptation_id,
                    "timestamp": adaptation.timestamp.isoformat()
                    if hasattr(adaptation.timestamp, "isoformat")
                    else str(adaptation.timestamp),
            ...
            if field in experience and experience[field]:
                try:
                    if isinstance(experience[field], str):
                        experience[field.replace("_json", "")] = _json_loads(
                            experience[field]
                        )
                    else:
    ...
    def _dict_to_experience(self, data: Dict[str, Any]) -> Experience:
        ...
        try:
            ...
            goal_understanding = data.get("goal_understanding") or data.get(
                "goal_understanding_json"
            )
            ...
            performance_metrics_data = data.get("performance_metrics") or data.get(
                "performance_metrics_json"
            )
            ...
            outcome = Outcome(
                ...
                success_level=SuccessLevel(outcome_data.get("success_level", "partial"))
                if isinstance(outcome_data, dict) and outcome_data.get("success_level")
                else SuccessLevel.PARTIAL,
                primary_achievements=outcome_data.get("primary_achievements", [])
                if isinstance(outcome_data, dict)
                else [],
                failures=outcome_data.get("failures", [])
                if isinstance(outcome_data, dict)
                else [],
                user_satisfaction=outcome_data.get("user_satisfaction")
                if isinstance(outcome_data, dict)
                else None,
            ...
            performance_metrics = PerformanceMetrics(
                goal_achievement_score=performance_metrics_data.get(
                    "goal_achievement_score", 0.0
                )
                if isinstance(performance_metrics_data, dict)
                else 0.0,
                quality_score=performance_metrics_data.get("quality_score", 0.0)
                if isinstance(performance_metrics_data, dict)
                else 0.0,
                efficiency_score=performance_metrics_data.get("efficiency_score", 0.0)
                if isinstance(performance_metrics_data, dict)
                else 0.0,
                token_usage=performance_metrics_data.get("token_usage", 0)
                if isinstance(performance_metrics_data, dict)
                else 0,
                execution_time=performance_metrics_data.get("execution_time", 0.0)
                if isinstance(performance_metrics_data, dict)
                else 0.0,
                error_count=performance_metrics_data.get("error_count", 0)
                if isinstance(performance_metrics_data, dict)
                else 0,
                adaptation_count=performance_metrics_data.get("adaptation_count", 0)
                if isinstance(performance_metrics_data, dict)
                else 0,
            )
