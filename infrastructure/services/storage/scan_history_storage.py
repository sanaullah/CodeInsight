...
class ScanHistoryStorage(BaseStorage):
    ...
    def save(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to serialize result to JSON: {json_error}", exc_info=True
                )
                # Try with serialization helper if available
                try:
                    from infrastructure.utils.scan_history import serialize_planning_result

                    serialized_result = (
                        serialize_planning_result(result) if result else {}
                    )
                    result_json = _json_dumps(serialized_result)
                except Exception as serialize_error:
                    logger.error(
                        f"Failed to serialize result even with helper: {serialize_error}",
                        exc_info=True,
                    )
                    raise StorageError(
                        f"Failed to serialize result: {json_error}"
                    ) from json_error
                ...
                with conn.cursor() as cursor:
                    ...
                    if result and isinstance(result, dict):
                        ...
                        if metrics_to_store:
                            ...
                            logger.debug(
                                f"Stored {len(metrics_to_store)} metrics for scan {scan_id}"
                            )

                conn.commit()

            logger.info(
                f"Saved scan {scan_id} (agent: {agent_name}, project: {project_path})"
            )
            return str(scan_id)
                ...
                with conn.cursor() as cursor:
                    ...
                    if "result_json" in scan and scan["result_json"]:
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse result_json for scan {scan_id}: {e}"
                            )
                            scan["result"] = {}
                    else:
                        scan["result"] = {}
    ...
    def delete(self, record_id: str) -> bool:
        ...
        try:
            ...
            if deleted:
                logger.info(
                    f"Deleted scan {scan_id} (and {metrics_deleted} related metrics)"
                )
            else:
    ...
    def get_by_project_path(
        self, project_path: str, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_by_agent_name(
        ...
        """
        return self.query(
            filters={"agent_name": agent_name}, limit=limit, order_by="timestamp DESC"
        )
        ...
        try:
            with get_db_connection(self.db_identifier, read_only=True) as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        f"SELECT AVG(files_scanned) FROM {self.schema}.scan_history"
                    )
                    result = cursor.fetchone()
    ...
    def get_analysis_metrics(self, scan_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error retrieving analysis metrics for scan {scan_id}: {e}",
                exc_info=True,
            )
            return []
