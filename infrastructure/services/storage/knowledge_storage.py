...
class KnowledgeStorage(BaseStorage):
    ...
    def save(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            file_hash = (
                ...
                else getattr(knowledge, "file_hash", None)
            )
            project_path = (
                data.get("project_path")
                if isinstance(data, dict)
                else getattr(knowledge, "project_path", None)
            )
                ...
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            if isinstance(knowledge.knowledge_type, KnowledgeType)
    ...
    def query(
        ...
        try:
            ...
            """).format(schema=sql.Identifier(self.schema), where=where_clause)
        ...
        try:
            ...
            if query:
                ...
                where_clauses.append(
                    "to_tsvector('english', content_json::text) @@ plainto_tsquery('english', %s)"
                )
                params.append(query)
            ...
            if knowledge_types:
                type_values = [
                    kt.value if isinstance(kt, KnowledgeType) else str(kt)
    ...
    def get_architecture_model(
        ...
        try:
            ...
            query = sql.SQL("""
                ...
                {order}
            """).format(
                schema=sql.Identifier(self.schema), where=where_clause, order=order_by
            )
    ...
    def _parse_json_fields(self, knowledge: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "knowledge_type" in knowledge and isinstance(
            knowledge["knowledge_type"], str
        ):
            try:
