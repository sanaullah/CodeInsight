...
class BaseStorage(ABC):
    """
    Abstract base class for all storage infrastructure.services.
    ...
    def bulk_delete(self, record_ids: List[str]) -> int:
        ...
        try:
            ...
            logger.debug(
                f"Failed to track storage bulk_delete operation: {track_error}"
            )
    ...
    def log_operation(
        ...
        if success:
            ...
            if record_id:
                logger.error(
                    f"{operation} operation failed for {record_id}: {error}",
                    exc_info=error,
                )
            else:
