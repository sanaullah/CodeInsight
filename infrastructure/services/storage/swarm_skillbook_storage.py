...
from domain.skills import SwarmSkill, SwarmReflection
...
class SwarmSkillbookStorage(BaseStorage):
    ...
    def save_skill(self, skill: SwarmSkill) -> str:
        ...
        try:
            ...
            architecture_type = (
                skill.context.get("architecture_type", "") if skill.context else ""
            )
    ...
    def get_relevant_skills(
        self, architecture_type: str, goal: Optional[str] = None
        ...
        try:
            ...
            skills = self.get_skills(context={"architecture_type": architecture_type})
            ...
            general_skills = [
                ...
                if not (s.context and s.context.get("architecture_type"))
            ]
    ...
    def update_skill_usage(
        self, skill_id: str, effectiveness: float, analysis_id: str
    ) -> None:
        """
        ...
        try:
            with get_db_connection(self.db_identifier, read_only=False) as conn:
                with conn.cursor() as cursor:
                    ...
                    new_success_rate = (
                        (current_success_rate * (new_usage - 1) + effectiveness)
                        / new_usage
                        if new_usage > 0
                        else effectiveness
                    )
    ...
    def save_reflection(self, reflection: SwarmReflection) -> str:
        ...
        try:
            ...
            metadata_json = (
                _json_dumps(reflection.metadata) if reflection.metadata else None
            )
    ...
    def get_similar_reflections(
        self, architecture_type: str, limit: int = 10
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    # Parse JSON fields
                    reflection_dict = self._parse_reflection_json_fields(
                        reflection_dict
                    )
    ...
    def deduplicate_skills(self) -> Dict[str, int]:
        ...
        try:
                    ...
                    if key in seen:
                        ...
                        with conn.cursor() as cursor:
                            ...
                            if existing_skill and current_skill:
                                ...
                                if total_usage > 0:
                                    combined_success = (
                                        existing_skill[1] * existing_skill[0]
                                        + current_skill[1] * current_skill[0]
                                    ) / total_usage
                                else:
                                    combined_success = max(
                                        existing_skill[1], current_skill[1]
                                    )
            ...
            logger.info(
                f"Deduplicated skills: {merged_count} merged, {deleted_count} deleted"
            )
        ...
        try:
            with get_db_connection(self.db_identifier, read_only=True) as conn:
                with conn.cursor() as cursor:
                    ...
                    # Total reflections
                    cursor.execute(
                        f"SELECT COUNT(*) FROM {self.schema}.swarm_reflections"
                    )
    ...
    def get_improvement_metrics(
        ...
        try:
            ...
            if first_half and second_half:
                ...
                quality_change = (
                    ((avg_quality_second - avg_quality_first) / avg_quality_first * 100)
                    if avg_quality_first > 0
                    else 0
                )

                avg_efficiency_first = sum(r[1] for r in first_half) / len(first_half)
                avg_efficiency_second = sum(r[1] for r in second_half) / len(
                    second_half
                )
                efficiency_change = (
                    (
                        (avg_efficiency_second - avg_efficiency_first)
                        / avg_efficiency_first
                        * 100
                    )
                    if avg_efficiency_first > 0
                    else 0
                )
                ...
                quality_trend = (
                    "improving"
                    if quality_change > 5
                    else ("declining" if quality_change < -5 else "stable")
                )
                efficiency_trend = (
                    "improving"
                    if efficiency_change > 5
                    else ("declining" if efficiency_change < -5 else "stable")
                )
            else:
                quality_trend = "stable"
                efficiency_trend = "stable"
    ...
    def _parse_reflection_json_fields(
        self, reflection_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Parse JSON fields in reflection dictionary."""
        if (
            "roles_effectiveness_json" in reflection_dict
            and reflection_dict["roles_effectiveness_json"]
        ):
            if isinstance(reflection_dict["roles_effectiveness_json"], str):
                reflection_dict["roles_effectiveness"] = _json_loads(
                    reflection_dict["roles_effectiveness_json"]
                )
            else:
                reflection_dict["roles_effectiveness"] = reflection_dict[
                    "roles_effectiveness_json"
                ]

        if (
            "prompt_quality_json" in reflection_dict
            and reflection_dict["prompt_quality_json"]
        ):
            if isinstance(reflection_dict["prompt_quality_json"], str):
                reflection_dict["prompt_quality"] = _json_loads(
                    reflection_dict["prompt_quality_json"]
                )
            else:
                reflection_dict["prompt_quality"] = reflection_dict[
                    "prompt_quality_json"
                ]

        if "metadata_json" in reflection_dict and reflection_dict["metadata_json"]:
            if isinstance(reflection_dict["metadata_json"], str):
                reflection_dict["metadata"] = _json_loads(
                    reflection_dict["metadata_json"]
                )
            else:
    ...
    def _dict_to_skill(self, data: Dict[str, Any]) -> SwarmSkill:
        ...
        if "context" not in data and "context_json" in data:
            data["context"] = (
                _json_loads(data["context_json"])
                if isinstance(data["context_json"], str)
                else data["context_json"]
            )
        if "metadata" not in data and "metadata_json" in data:
            data["metadata"] = (
                _json_loads(data["metadata_json"])
                if isinstance(data["metadata_json"], str)
                else (data["metadata_json"] or {})
            )
    ...
    def _dict_to_reflection(self, data: Dict[str, Any]) -> SwarmReflection:
        ...
        if "roles_effectiveness" not in data and "roles_effectiveness_json" in data:
            data["roles_effectiveness"] = (
                _json_loads(data["roles_effectiveness_json"])
                if isinstance(data["roles_effectiveness_json"], str)
                else data["roles_effectiveness_json"]
            )
        if "prompt_quality" not in data and "prompt_quality_json" in data:
            data["prompt_quality"] = (
                _json_loads(data["prompt_quality_json"])
                if isinstance(data["prompt_quality_json"], str)
                else data["prompt_quality_json"]
            )
        if "metadata" not in data and "metadata_json" in data:
            data["metadata"] = (
                _json_loads(data["metadata_json"])
