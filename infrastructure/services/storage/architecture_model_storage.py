...
from infrastructure.services.storage.knowledge_storage import KnowledgeStorage
...
from domain.collaboration import Knowledge, KnowledgeType
...
# MinIO support
try:
    from infrastructure.services.minio_client import MinIOClient
...
class ArchitectureModelStorage(BaseStorage):
    ...
    def store_architecture_model(
        ...
        if use_minio:
            ...
            try:
                ...
                # Upload to MinIO
                object_name = (
                    f"architecture_models/{project_path}/{file_hash}/v{version}.json"
                )
                self.minio_client.upload_fileobj(BytesIO(model_bytes), object_name)
                ...
                logger.warning(
                    f"Failed to store in MinIO, falling back to PostgreSQL: {e}"
                )
    ...
    def get(self, record_id: str) -> Optional[Dict[str, Any]]:
        ...
        version_str = parts[3]
        version = (
            int(version_str.replace("v", "")) if version_str.startswith("v") else 1
        )

        return self.get_architecture_model(project_path, file_hash, version)

    def get_architecture_model(
        self, project_path: str, file_hash: str, version: Optional[int] = None
    ...
    def query(
        ...
        results = self.knowledge_storage.query(
            filters=query_filters, limit=limit, offset=offset
        )
    ...
    def store_large_model(
        self, project_path: str, model_data: bytes, file_hash: str
    ) -> str:
        """
        ...
        try:
            ...
            self.minio_client.upload_fileobj(BytesIO(model_data), object_name)
    ...
    def check_model_size(self, model: Any) -> int:
        ...
        try:
            if isinstance(model, dict):
                ...
                model_json = _json_dumps(
                    model.__dict__ if hasattr(model, "__dict__") else str(model)
                )
