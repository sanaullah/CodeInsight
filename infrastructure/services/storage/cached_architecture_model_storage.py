...
from infrastructure.services.storage.architecture_model_storage import ArchitectureModelStorage
...
class CachedArchitectureModelStorage(CachedStorage):
        ...
        if model is not None:
            try:
                ...
                if metadata.get("stored_in") == "minio":
                    ...
                    logger.debug(
                        f"Cached architecture model: {record_id} with TTL {ttl}"
                    )
            except Exception as e:
    ...
    def store_large_model(
        ...
        knowledge_id = self.storage.store_large_model(
            project_path=project_path, model_data=model_data, file_hash=file_hash
        )
