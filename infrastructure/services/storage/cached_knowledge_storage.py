...
from domain.collaboration import KnowledgeType
...
class CachedKnowledgeStorage(CachedStorage):
    ...
    def search_knowledge(
        ...
        cache_key = self._get_search_cache_key(
            query, knowledge_types, agents, date_range, limit
        )
    ...
    def cleanup_expired_knowledge(self) -> int:
        ...
        if count > 0:
            try:
                ...
                logger.debug(
                    f"Invalidated all knowledge caches after cleanup ({count} items)"
                )
            except Exception as e:
    ...
    def _get_search_cache_key(
        ...
        key_data = {
            "query": query,
            "types": [
                kt.value if hasattr(kt, "value") else str(kt)
                for kt in (knowledge_types or [])
            ],
    ...
    def get_architecture_model(
        ...
        try:
            ...
            logger.warning(
                f"Invalid cache key for architecture model: {e}, using sanitized key"
            )
        ...
        try:
            ...
            if cached is not None:
                logger.debug(
                    f"Cache hit for architecture model: {project_path}:{file_hash}"
                )
                return cached
        except Exception as e:
