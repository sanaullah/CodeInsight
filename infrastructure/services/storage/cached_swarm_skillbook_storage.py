...
from infrastructure.services.storage.swarm_skillbook_storage import SwarmSkillbookStorage
from infrastructure.services.storage.cached_storage import CachedStorage
from infrastructure.services.cache_config import get_ttl_for_cache_type
from infrastructure.services.redis_cache import get_cache
from infrastructure.services.cache_utils import validate_cache_key
...
class CachedSwarmSkillbookStorage(CachedStorage):
    ...
    def get_skills(
        ...
        return self.storage.get_skills(
            skill_type=skill_type, skill_category=skill_category, context=context
        )
    ...
    def save_reflection(self, reflection: SwarmReflection) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Invalid cache key for reflection: {e}, using sanitized key"
            )
    ...
    def get_similar_reflections(
        ...
        try:
            ...
            logger.warning(
                f"Invalid cache key for reflections: {e}, using sanitized key"
            )
            # Re-raise to fail fast - caller should handle invalid keys
            raise
    ...
    def get_improvement_metrics(self, time_period: Tuple[Any, Any]) -> Dict[str, Any]:
