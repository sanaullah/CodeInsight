...
from infrastructure.services.db_config import get_db_config, RedisConfig
from infrastructure.services.redis_client import RedisClient
...
class RedisCache:
    ...
    def _validate_cache_key(self, key: str) -> str:
        ...
        if len(key) > 500:
            raise ValueError(
                f"Cache key exceeds maximum length of 500 characters (got {len(key)})"
            )
    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        try:
            from infrastructure.utils.langfuse.db_tracking import track_cache_operation
                ...
                try:
                    ...
                    if data is None:
                        ...
                        if obs:
                            try:
                                obs.update(
                                    output={"hit": False}, metadata={"hit": False}
                                )
                            except Exception:
                                pass
                        return default
    ...
    def get_with_lock(
        ...
        if lock_acquired:
            ...
            return self.get(key, default=None) or self.get_with_lock(
                key, compute_func, ttl, lock_ttl
            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.warning(
                    f"Invalid cache key for {func.__name__}: {e}, using sanitized key"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Cache get failed for {func.__name__}: {e}, computing result"
                )
