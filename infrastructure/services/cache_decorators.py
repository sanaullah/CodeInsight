    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self):
            ...
            if not hasattr(self, cache_key_attr):
                ...
                instance_id = id(self)
                setattr(
                    self, cache_key_attr, f"{class_name}:{func.__name__}:{instance_id}"
                )
            ...
            try:
                ...
                if cached is not None:
                    ...
                    if remaining_ttl is not None and remaining_ttl < (stale_ttl - ttl):
                        ...
                        logger.debug(
                            f"Cache entry for {func.__name__} is stale, refreshing in background"
                        )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Background refresh failed for {func.__name__}: {e}"
                            )
                    return cached
            except Exception as e:
                logger.warning(f"Cache get failed for {func.__name__}: {e}")
