...
`infrastructure/observability/langfuse_stack/` directory with your initial
credentials:
...
1. **Change to the Langfuse stack directory** (where `docker-compose.yml` lives):

   ```powershell
   cd infrastructure/observability/langfuse_stack
   ```
