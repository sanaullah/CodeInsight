...
- `infrastructure/llm/` folder (exclude `__pycache__/`)
- `infrastructure/utils/` folder (exclude `__pycache__/`)
...
The `.env` file in `infrastructure/observability/langfuse_stack/` is only for Docker Compose to run the Langfuse server itself, not for the framework.
...
```python
from infrastructure.llm.config import ConfigManager, ensure_env_ready
...
```python
from infrastructure.utils import create_langfuse_trace
...
```python
from infrastructure.utils import get_langfuse_client
...
1. Replace `from infrastructure.llm import OpenAI` with `import litellm`
