...
class PromptTrackingConfig(BaseModel):
    ...
    store_in_postgresql: bool = (
        True  # Store prompts in PostgreSQL (always enabled as fallback)
    )
...
class LangfuseConfig(BaseModel):
    ...
    prompt_tracking: PromptTrackingConfig = Field(
        default_factory=PromptTrackingConfig
    )  # Prompt tracking config


class CheckpointConfig(BaseModel):
    """Checkpoint persistence configuration for workflow."""
...
class HumanInLoopConfig(BaseModel):
    """Human-in-the-loop configuration for workflow."""
...
class StreamingConfig(BaseModel):
    """Streaming configuration for workflow."""
...
class LoggingConfig(BaseModel):
    ...
    module_loggers: Optional[Dict[str, Any]] = Field(
        default_factory=lambda: {"enabled": True, "modules": ["langfuse", "workflows"]}
    )
...
class ConfigManager:
    ...
    DEFAULT_MODELS = {
        "deepseek-coder-6.7b": ModelConfig(
            name="deepseek-coder-6.7b", max_input_tokens=16384, max_output_tokens=4096
        ),
        "deepseek-coder-33b": ModelConfig(
            name="deepseek-coder-33b", max_input_tokens=16384, max_output_tokens=4096
        ),
        "glm-4-9b": ModelConfig(
            name="glm-4-9b", max_input_tokens=32768, max_output_tokens=8192
        ),
        "qwen2.5-7b": ModelConfig(
            name="qwen2.5-7b", max_input_tokens=32768, max_output_tokens=8192
        ),
        "qwen2.5-14b": ModelConfig(
            name="qwen2.5-14b", max_input_tokens=32768, max_output_tokens=8192
        ),
        "qwen2.5-32b": ModelConfig(
            name="qwen2.5-32b", max_input_tokens=32768, max_output_tokens=8192
        ),
        "kimi-8b": ModelConfig(
            name="kimi-8b", max_input_tokens=128000, max_output_tokens=16384
        ),
        "deepseek-v2.5": ModelConfig(
            name="deepseek-v2.5", max_input_tokens=128000, max_output_tokens=16384
        ),
        "qwen2.5-72b": ModelConfig(
            name="qwen2.5-72b", max_input_tokens=128000, max_output_tokens=16384
        ),
        "glm-4-9b-1m": ModelConfig(
            name="glm-4-9b-1m", max_input_tokens=1000000, max_output_tokens=16384
        ),
    }
                    ...
                    if key == "models" and isinstance(value, dict):
                        ...
                        for model_name, model_data in value.items():
                            if isinstance(model_data, dict):
                                default_config.models[model_name] = ModelConfig(
                                    **model_data
                                )
                        ...
                        if "checkpoint" in workflows_data and isinstance(
                            workflows_data["checkpoint"], dict
                        ):
                            workflows_data["checkpoint"] = CheckpointConfig(
                                **workflows_data["checkpoint"]
                            )
                        if "human_in_loop" in workflows_data and isinstance(
                            workflows_data["human_in_loop"], dict
                        ):
                            workflows_data["human_in_loop"] = HumanInLoopConfig(
                                **workflows_data["human_in_loop"]
                            )
                        if "streaming" in workflows_data and isinstance(
                            workflows_data["streaming"], dict
                        ):
                            workflows_data["streaming"] = StreamingConfig(
                                **workflows_data["streaming"]
                            )
...
def ensure_env_ready() -> None:
    ...
    if hasattr(config, "logging") and config.logging.enabled:
        try:
            from infrastructure.utils.config.logging_config import setup_logging_from_config
    ...
    print(
        f"LiteLLM environment ready - API Base: {config.api_base[:50]}..., Model: {config.default_model}"
    )
