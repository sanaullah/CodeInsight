...
try:
    ...
    # Get the relocated workflow module path from the project root.
    project_root = Path(__file__).resolve().parents[2]
    workflows_path = project_root / "workflow" / "nodes.py"
    
    if workflows_path.exists():
        spec = importlib.util.spec_from_file_location("workflow.nodes", workflows_path)
        workflows_nodes = importlib.util.module_from_spec(spec)
        sys.modules["workflow.nodes"] = workflows_nodes
...
from infrastructure.utils.knowledge_base import get_architecture_model, store_architecture_model
