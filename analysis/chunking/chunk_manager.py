...
class ChunkManager:
    ...
    def create_chunks(self, files: List[FileInfo]) -> List[Chunk]:
            ...
            if current_tokens + file_tokens <= self.max_tokens:
                ...
                if current_chunk_files:
                    chunk = self._create_chunk(
                        chunk_index, current_chunk_files, current_tokens
                    )
                    chunks.append(chunk)
                    chunk_index += 1
        ...
        logger.info(
            f"Created {len(chunks)} chunks from {len(files)} files using {self.chunking_strategy.value} strategy"
        )
        return chunks
    ...
    def _create_chunk(
        self, chunk_index: int, file_chunks: List[FileChunk], total_tokens: int
    ) -> Chunk:
        ...
        for i, line in enumerate(lines):
            ...
            if current_tokens + line_tokens <= self.max_tokens:
                ...
                if current_lines:
                    ...
                    chunk = self._create_chunk(
                        chunk_index, [file_chunk], current_tokens
                    )
                    chunks.append(chunk)
                    chunk_index += 1
