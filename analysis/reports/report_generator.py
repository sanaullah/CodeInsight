...
def generate_markdown_report(
    analysis_result: Dict[str, Any], project_name: Optional[str] = None
) -> str:
    """
    ...
    try:
        ...
        project_name = project_name or analysis_result.get(
...
"""
        ...
        if "analysis_results" in analysis_result:
            ...
            for i, chunk in enumerate(
                analysis_result["analysis_results"][:10], 1
            ):  # Limit to first 10
                report += f"### Chunk {i}\n\n"
...
"""
        ...
        if "chunk_results" in agent_result and agent_result["chunk_results"]:
            ...
            for i, chunk_result in enumerate(
                agent_result["chunk_results"][:10], 1
            ):  # Limit to first 10
                report += f"### Chunk {i}\n\n"
...
"""
        ...
        for role_name, agent_result in individual_agent_results.items():
            ...
            # Add agent's synthesized report
            if (
                "synthesized_report" in agent_result
                and agent_result["synthesized_report"]
            ):
                report += f"{agent_result['synthesized_report']}\n\n"
            else:
                report += "*No report available for this agent.*\n\n"
            ...
            report += f"**Status:** {agent_result.get('status', 'unknown')}  \n"
            report += (
                f"**Chunks Analyzed:** {agent_result.get('chunks_analyzed', 0)}\n\n"
            )

            # Add chunk results summary if available
            if "chunk_results" in agent_result and agent_result["chunk_results"]:
                report += (
                    f"**Chunks Processed:** {len(agent_result['chunk_results'])}\n\n"
                )
