...
from infrastructure.utils.io.async_io import async_ast_parse
...
class DependencyResolver:
    ...
    def detect_dependencies(
        self, selected_dirs: List[str], project_root: str, files: List[FileInfo]
    ) -> Dict[str, List[str]]:
        """
        ...
        dependencies: Dict[str, List[str]] = {
            dir_path: [] for dir_path in selected_dirs
        }
        ...
        for file_info in files:
            ...
            for import_name in imports:
                target_dir = self._map_import_to_directory(
                    import_name, file_path, project_root_path, available_dirs
                )
    ...
    def resolve_dependencies(
            ...
            try:
                if Path(d).is_absolute():
                    normalized_selected.append(
                        str(Path(d).relative_to(project_root_path))
                    )
                else:
        ...
        if files is None or not files:
            logger.warning(
                "Files not provided to resolve_dependencies, cannot detect dependencies"
            )
        ...
        while to_process and depth < self.max_depth:
            ...
            for dir_path in current_level:
                ...
                deps = self._find_dependencies_for_directory(
                    dir_path, project_root_path, files
                )
    ...
    def _parse_imports(self, file_info: FileInfo, project_root: Path) -> Set[str]:
        ...
        if extension == ".py":
            return self._parse_python_imports(
                file_info.content, file_path, project_root
            )
    ...
    def _parse_python_imports(
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        # Get first part of import (e.g., "utils" from "infrastructure.utils.langfuse")
                        module_name = alias.name.split(".")[0]
                        imports.add(module_name)
                elif isinstance(node, ast.ImportFrom):
                    if node.module:
                        # Get first part of module (e.g., "utils" from "infrastructure.utils.langfuse")
            ...
            logger.debug(
                f"Syntax error parsing {file_path}: {e}, falling back to regex"
            )
    ...
    def _map_import_to_directory(
        ...
        if import_name.startswith("_") or import_name in [
            "sys",
            "os",
            "json",
            "re",
    ...
    def _get_directory_for_file(
        self, file_path: Path, selected_dirs: List[str]
    ) -> Optional[str]:
        """
    ...
    def _find_dependencies_for_directory(
        self, dir_path: str, project_root: Path, files: List[FileInfo]
    ) -> List[str]:
        """
        ...
        for file_info in files:
            ...
            for import_name in imports:
                target_dir = self._map_import_to_directory(
                    import_name, file_path, project_root, available_dirs
                )
