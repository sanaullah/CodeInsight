...
class ProjectScanner:
    ...
    def __init__(
        ...
        self.file_extensions = (
            file_extensions if file_extensions is not None else [".py"]
        )
    ...
    def scan_directory(
        ...
        extensions = (
            file_extensions if file_extensions is not None else self.file_extensions
        )
        ...
        if self.auto_detect_languages and not file_extensions:
            try:
                ...
                if detected_languages:
                    ...
                    extensions = get_extensions_for_languages(
                        list(detected_languages.keys())
                    )
                    logger.info(
                        f"Auto-detected languages: {list(detected_languages.keys())}, extensions: {extensions}"
                    )
            except ImportError:
                logger.warning(
                    "LanguageDetector not available, using default extensions"
                )
    ...
    def scan_directories(
        ...
        logger.info(
            f"Scanned {len(directories)} directories, found {len(all_files)} unique files"
        )
        return all_files
    ...
    def extract_documentation_metadata(
        self, file_paths: List[Path], project_root: Optional[Path] = None
    ) -> Dict[str, Any]:
        """
        ...
        for file_path in file_paths:
            try:
                ...
                rel_path = (
                    ...
                    if project_root and project_root in file_path.parents
                    else str(file_path)
                )
                ...
                if file_name_upper.startswith("README"):
                    ...
                    file_name_upper.startswith("LICENSE")
                    or file_name_upper == "LICENCE"
                ):
                    metadata["license_files"].append(rel_path)
                    metadata["documentation_files"].append(rel_path)
                elif file_path.parent.name.lower() == "docs" or "docs" in str(
                    file_path.parent
                ):
                    metadata["documentation_files"].append(rel_path)
                ...
                if file_path.suffix in self.file_extensions:
                    ...
                    if language_obj:
                        ...
                        try:
                            from parsers.multi_language_parser import (
                                MultiLanguageParser,
                            )
                            ...
                            if ast_metadata:
                                metadata["total_functions"] += ast_metadata.get(
                                    "total_functions", 0
                                )
                                metadata["total_classes"] += ast_metadata.get(
                                    "total_classes", 0
                                )
                                metadata["documented_functions"] += ast_metadata.get(
                                    "documented_functions", 0
                                )
                                metadata["documented_classes"] += ast_metadata.get(
                                    "documented_classes", 0
                                )

                                # Count docstrings from documentation metadata
                                doc_metadata = ast_metadata.get("documentation", {})
                                metadata["docstring_count"] += doc_metadata.get(
                                    "docstring_count", 0
                                )
                        except ImportError:
                            ...
                            if file_path.suffix == ".py":
                                try:
                                    ...
                                    logger.debug(
                                        f"Error parsing Python file {file_path}: {e}"
                                    )
                        except Exception as e:
                            logger.debug(
                                f"Error parsing file {file_path} with MultiLanguageParser: {e}"
                            )
        ...
        documented_items = (
            metadata["documented_functions"] + metadata["documented_classes"]
        )
        if total_items > 0:
    ...
    def extract_dependency_metadata(self, directory_path: str) -> Dict[str, Any]:
        """
    ...
    def register_invalidation_hook(self, hook) -> None:
        ...
        """
        from infrastructure.services.cache_invalidation_hooks import FileChangeHook
    ...
    def trigger_file_change_invalidation(
        self, project_path: str, file_hash: Optional[str] = None
    ) -> None:
        """
        ...
        if self._invalidation_hook:
            event_data = {"project_path": project_path, "file_hash": file_hash}
